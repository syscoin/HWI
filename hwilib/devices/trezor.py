...
class TrezorClient(HardwareWalletClient):
    ...
    def _prepare_device(self) -> None:
        self.coin_name = 'Syscoin' if self.chain == Chain.MAIN else 'Testnet'
    ...
    def get_pubkey_at_path(self, path: str) -> ExtendedKey:
        ...
        try:
            ...
            raise BadArgumentError(str(e))
        output = syscoin.get_public_node(self.client, expanded_path, coin_name=self.coin_name)
    ...
    def sign_tx(self, tx: PSBT) -> PSBT:
        ...
        master_key = syscoin.get_public_node(self.client, [0x80000000], coin_name='Syscoin')
    ...
    def sign_message(self, message: Union[str, bytes], keypath: str) -> str:
        ...
        result = syscoin.sign_message(self.client, self.coin_name, path, message)
