...
def signtx(client: HardwareWalletClient, psbt: str) -> Dict[str, Union[bool, str]]:
    """
    Sign a Partially Signed Syscoin Transaction (PSBT) with the client.
...
def signmessage(client: HardwareWalletClient, message: str, path: str) -> Dict[str, str]:
    ...
    The message will be signed using the Syscoin signed message standard used by Syscoin Core.
...
def getkeypool(
    ...
    """
    Get a dictionary which can be passed to Syscoin Core's ``importmulti`` or ``importdescriptors`` RPCs to import a watchonly wallet based on the client.
    ...
    :param end: The end of the range to import, inclusive
    :param internal: Whether the dictionary should indicate that the descriptor should be for change addresses
    :param keypool: Whether the dictionary should indicate that the dsecriptor should be added to the Syscoin Core keypool/addresspool
    :param account: The BIP 44 account to use if ``path`` is not specified
