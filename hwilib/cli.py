...
def process_commands(cli_args):
    ...
    getmasterxpub_parser = subparsers.add_parser('getmasterxpub', help='Get the extended public key at m/84\'/57\'/0\'')
    getmasterxpub_parser.set_defaults(func=getmasterxpub_handler)

    signtx_parser = subparsers.add_parser('signtx', help='Sign a PSBT')
    signtx_parser.add_argument('psbt', help='The Partially Signed Syscoin Transaction to sign')
    ...
    getkeypool_parser = subparsers.add_parser('getkeypool', help='Get JSON array of keys that can be imported to Syscoin Core with importmulti')
    ...
    getkeypool_parser.add_argument('--sh_wpkh', action='store_true', help='Generate p2sh-nested segwit addresses (default path: m/49h/57h/0h/[0,1]/*)')
    getkeypool_parser.add_argument('--wpkh', action='store_true', help='Generate bech32 addresses (default path: m/84h/57h/0h/[0,1]/*)')
    getkeypool_parser.add_argument('--account', help='BIP43 account', type=int, default=0)
    getkeypool_parser.add_argument('--path', help='Derivation path, default follows BIP43 convention, e.g. m/84h/57h/0h/1/* with --wpkh --internal. If this argument and --internal is not given, both internal and external keypools will be returned.')
    ...
    group.add_argument('--desc', help='Output Descriptor. E.g. wpkh([00000000/84h/57h/0h]xpub.../0/0), where 00000000 must match --fingerprint and xpub can be obtained with getxpub. See doc/descriptors.md in Syscoin Core')
    group.add_argument('--path', help='The BIP 32 derivation path of the key embedded in the address, default follows BIP43 convention, e.g. m/84h/57h/0h/1/*')
